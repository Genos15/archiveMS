...
class AdminMapper(
...
) : MapperState<Row, Any, Admin> {

    override fun list(row: Row, o: Any): List<Admin> = try {
    ...
    override fun factory(row: Row, o: Any): Optional<Admin> = try {
        val payload = row.get(0, String::class.java) ?: error("something went wrong with mapping value")
        Optional.ofNullable(mapper.readValue<List<Admin>?>(payload)?.firstOrNull())
    } catch (e: Exception) {
        logger.warn { e.message }
