...
class ModeMapper(
...
) : MapperState<Row, Any, Mode> {

    override fun list(row: Row, o: Any): List<Mode> = try {
    ...
    override fun factory(row: Row, o: Any): Optional<Mode> = try {
        val payload = row.get(0, String::class.java) ?: error("something went wrong with mapping value")
        Optional.ofNullable(mapper.readValue<List<Mode>?>(payload)?.firstOrNull())
    } catch (e: Exception) {
        logger.warn { e.message }
        Optional.empty<Mode>()
    }
