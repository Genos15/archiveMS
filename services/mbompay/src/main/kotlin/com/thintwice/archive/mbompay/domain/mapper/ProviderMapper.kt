...
class ProviderMapper(
...
) : MapperState<Row, Any, Provider> {

    override fun list(row: Row, o: Any): List<Provider> = try {
    ...
    override fun factory(row: Row, o: Any): Optional<Provider> = try {
        val payload = row.get(0, String::class.java) ?: error("something went wrong with mapping value")
        Optional.ofNullable(mapper.readValue<List<Provider>?>(payload)?.firstOrNull())
    } catch (e: Exception) {
        logger.warn { e.message }
