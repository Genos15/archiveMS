...
class JwtTokenMapper(
...
) : MapperState<Row, Any, JwtToken> {

    override fun list(row: Row, o: Any): List<JwtToken> = try {
    ...
    override fun factory(row: Row, o: Any): Optional<JwtToken> = try {
        val payload = row.get(0, String::class.java) ?: error("something went wrong with mapping value")
        Optional.ofNullable(mapper.readValue<List<JwtToken>?>(payload)?.firstOrNull())
    } catch (e: Exception) {
        logger.warn { e.message }
        Optional.empty<JwtToken>()
    }
}